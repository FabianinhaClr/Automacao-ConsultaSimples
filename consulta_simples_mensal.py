...
OUTPUT_FILE = "resultado_simples1.xlsx"
SLEEP = 0.5
DEBUG = True
...
def is_month_fully_covered(periods, year, month):
    ...
    for p in periods:
        ...
        # Só considera se o período começa antes ou no início do mês E termina depois ou no final do mês
    ...
    # Se nenhum período cobre o mês completo
    # Procura se há exclusão dentro do mês
    for p in periods:
        si = p.get("start")
        ei = p.get("end")
        if not si or not ei:
...
def main():
    ...
    for cnpj in tqdm(cnpjs, desc="Consultando CNPJs"):
        ...
        # ==========================
        # SITUAÇÃO ATUAL + COMPLETAR PERÍODOS
        # ==========================
        situacao_atual = None
        ...
        if (
            ...
            and "nao optante" not in texto_situacao
        ):
